...
function ServicesPage(props) {
  ...
  useEffect(() => {
    ...
    .then(data => {
      const userFams = data.filter(el => el.username === props.local_user.username)
    ...
    .then(() => {
      ...
      .then(data => {
        ...
        const familyServices = data.filter(el => props.extFamily.includes(el.family_name));
        extFamilyService = familyServices;
        const servicesArr = [];
        for (let i = 0; i < extFamilyService.length; i++) {
          servicesArr.push(<ServicesDisplay
            key={i} 
